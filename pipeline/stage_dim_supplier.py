...
stg_supplier_df = get_wwi_db_dataframe(control_metadata["datasource_name"], sp_params)

# COMMAND ----------

# Add lineage key for reference.
stg_supplier_df = stg_supplier_df.withColumn("lineage_key", f.lit(lineage_key).cast("bigint"))

# COMMAND ----------

# Insert overwrite.
stg_supplier_df.write.format("delta").mode("overwrite").saveAsTable(f"wwi_stg.{stg_table_name}")
