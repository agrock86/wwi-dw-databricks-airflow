...
stg_transaction_type_df = spark.table(f"wwi_stg.{stg_table_name}")
closeoff_df = spark.table(f"wwi_stg.{stg_table_name}") \
    .groupBy(pk_column_name).agg(f.min("valid_from").alias("valid_from"))

# COMMAND ----------

dim_transaction_type_dt = DeltaTable.forName(spark, f"wwi_dim.{dim_table_name}")
...
dim_transaction_type_dt.alias("target") \
...
# Insert new records.
dim_transaction_type_dt.alias("target") \
    .merge(stg_transaction_type_df.alias("source"), f"target.{pk_column_name} = source.{pk_column_name}") \
