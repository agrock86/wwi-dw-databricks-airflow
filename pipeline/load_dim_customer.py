...
stg_customer_df = spark.table(f"wwi_stg.{stg_table_name}")
closeoff_df = spark.table(f"wwi_stg.{stg_table_name}") \
    .groupBy(pk_column_name).agg(f.min("valid_from").alias("valid_from"))

# COMMAND ----------

dim_customer_dt = DeltaTable.forName(spark, f"wwi_dim.{dim_table_name}")
...
# Insert new records.
dim_customer_dt.alias("target") \
    .merge(stg_customer_df.alias("source"), f"target.{pk_column_name} = source.{pk_column_name}") \
    .whenNotMatchedInsert(
        values = {
            pk_column_name: f"source.{pk_column_name}",
            "customer": "source.customer",
            "bill_to_customer": "source.bill_to_customer",
            "category": "source.category",
            "buying_group": "source.buying_group",
