...
stg_payment_method_df = spark.table(f"wwi_stg.{stg_table_name}")
closeoff_df = spark.table(f"wwi_stg.{stg_table_name}") \
    .groupBy(pk_column_name).agg(f.min("valid_from").alias("valid_from"))

# COMMAND ----------

dim_payment_method_dt = DeltaTable.forName(spark, f"wwi_dim.{dim_table_name}")
...
dim_payment_method_dt.alias("target") \
...
# Insert new records.
dim_payment_method_dt.alias("target") \
    .merge(stg_payment_method_df.alias("source"), f"target.{pk_column_name} = source.{pk_column_name}") \
