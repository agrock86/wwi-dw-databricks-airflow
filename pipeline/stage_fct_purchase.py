...
stg_purchase_df = get_wwi_db_dataframe(control_metadata["datasource_name"], sp_params)

# COMMAND ----------

# Add lineage key for reference.
# Add columns for dimension keys.
stg_purchase_df = stg_purchase_df.withColumn("lineage_key", f.lit(lineage_key).cast("bigint")) \
...
# Insert overwrite.
stg_purchase_df.write.format("delta").mode("overwrite").saveAsTable(f"wwi_stg.{stg_table_name}")
